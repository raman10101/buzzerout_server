...
class RegisterQuery
{
	...
	public function updateUserInRegister($first_name, $last_name, $username, $email, $password)
	{
		...
		// $unique = md5($unique);
		$link .= "?id=" . $unique;
		$stmt = mysqli_query($this->conn, "UPDATE register SET firstname = '" . $first_name . "', lastname= '" . $last_name . "',activation_link='".$unique."',  username - '" . $username . ", password ='" . $password . "', valid_till = ,DATE_ADD(NOW(), INTERVAL + 6 DAY)) WHERE email = '" . $email . "'");
		if ($stmt) {
