...
class FeedQuery
{
	...
	public function fetchFeedByRole($role)
	{
		$response = array();
		$response["feed"] = array();
		$stmt = mysqli_query($this->conn, "select feed.* from feed left join buzz_hide on feed.feed_id=buzz_hide.buzz_id where buzz_hide.buzz_id is NULL order by timestamp DESC");
		if (mysqli_num_rows($stmt) > 0) {
			$response["error"] = false;
			$response["message"] = "Feed Found";
			while ($row = mysqli_fetch_assoc($stmt)) {
